...
def puploader_upload():
    if "username" in session:
        folders = [folder for folder in os.listdir(app.config['UPLOAD_FOLDER'])
...
def create_new_folder():
    if app.config['PRIVATE']:
        new_folder = request.form['folderName'].replace('.',
                                                        '').replace('/',
                                                                    '').replace('\\', '')
...
def render_subfolder_gallery(subfolder):
    ...
    if "username" in session:
        ...
        folders.sort()
        photos = [subfolder + '/' + photo for photo in os.listdir(subfolder_dir)
                  if '.' in photo]
